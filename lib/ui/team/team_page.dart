...
class _TeamState extends StateWithBloC<TeamPage, TeamBloC> {
        ...
        StreamBuilder<bool>(
          ...
          builder: (context, snapshotLeading) {
            return TXMainAppBarWidget(
              ...
              floatingActionButton: bloc.teamIsEnterprise ? null : FloatingActionButton(
                backgroundColor: R.color.secondaryColor,
                onPressed: () {
                  NavigationUtils.push(
                      context, OnboardingWelcomePage()).then((res) {
              ...
              body: StreamBuilder<List<TeamModel>>(
                ...
                builder: (ctx, snapshot) {
                  snapshot.data!.sort(
                      (e1, e2) => e1.titleFixed.toLowerCase().compareTo(e2.titleFixed.toLowerCase()));
  ...
  void _showDialogLogout({required BuildContext context}) {
    txShowWaringDialogMaterial(context,
      ...
      if (action) {
        bloc.logout().then((value) => NavigationUtils.pop(context, result: true));
      }
    },
