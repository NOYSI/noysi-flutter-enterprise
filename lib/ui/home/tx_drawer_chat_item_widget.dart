...
class TXDrawerChatItemWidget extends StatefulWidget {
  ...
  const TXDrawerChatItemWidget(
      {Key? key, required this.drawerChatModel, required this.onTap})
      : super(key: key);

  @override
  _TXDrawerChatItemWidgetState createState() => _TXDrawerChatItemWidgetState();
}
...
class _TXDrawerChatItemWidgetState extends State<TXDrawerChatItemWidget> {
  ...
  Widget _getHeaderDrawerChatWidget(TeamColors colors) {
    ...
    return Container(
      margin: EdgeInsets.only(top: isThread ? 0 : 20),
              ...
              Positioned(
                child: TXUserPresenceWidget(
                  userPresence: _drawerChatModel.memberModel?.userPresence ??
                      UserPresence.out,
                  isUserEnabled: _drawerChatModel.memberModel?.active == true,
                ),
              )
            ],
          ),
                    ...
                    children: <Widget>[
                      ...
                      Row(
                        children: [
                          Flexible(
                            child: TXTextWidget(
                              text: _drawerChatModel.memberModel?.active ==
                                      false
                                  ? R.string.inactiveMember
                                  : "${CommonUtils.getMemberNameSingle(_drawerChatModel.memberModel) ?? ""}",
                              color:
                                  _drawerChatModel.memberModel?.active == false
                                      ? Colors.redAccent
                                      : _drawerChatModel.isSelected
                                          ? colors.activeItemText
                                          : colors.subtextColor,
                              fontWeight: _drawerChatModel.memberModel?.active == false ? FontWeight.normal : FontWeight.w500,
                              maxLines: 1,
                              fontStyle:
                                  _drawerChatModel.memberModel?.active == false
                                      ? FontStyle.italic
