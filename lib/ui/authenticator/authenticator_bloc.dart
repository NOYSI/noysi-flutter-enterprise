...
import '../../enums.dart';
import '../_base/bloc_form_validator.dart';

class AuthenticatorBloC extends BaseBloC with LoadingBloC, FormValidatorBloC, ErrorHandlerBloC {
  ...
  BehaviorSubject<List<AuthenticatedEntity>> _dataController =
      new BehaviorSubject.seeded([]);

  Stream<List<AuthenticatedEntity>> get dataResult => _dataController.stream;

  BehaviorSubject<int> _timerController = new BehaviorSubject.seeded(30);
  ...
  void generateCodes(List<AuthenticatedEntity> list) {
    ...
    expirationCountdown = Timer.periodic(Duration(seconds: 1), (time) {
  ...
  void processQrResult(String result) {
    ...
    if (match != null) {
      ...
      try {
        if(label.isNotEmpty && secret.isNotEmpty && base32.decodeAsString(secret).isNotEmpty) createApp(issuer, label, type, secret);
        else showErrorMessageFromString(R.string.labelTextWarning);
      } catch (ex) {
  ...
  Future<void> createApp(
    ...
    if (res is ResultSuccess<AuthenticatedEntity>) {
      list.add(res.value);
      generateCodes(list);
    }
    isLoading = false;
